...
#)  chore: add a script to bump-version (:pr:`3260`) :user:`marcelotduarte`
#)  chore: switch to prek (:pr:`3259`) :user:`marcelotduarte`
#)  chore: improve integration with 'uv' (:pr:`3258`) :user:`marcelotduarte`
#)  hooks: improve pytz to work w/ setuptools 82+ in zip mode (:pr:`3257`) :user:`marcelotduarte`
#)  chore: support setuptools v81 and v82 (:pr:`3256`) :user:`marcelotduarte`
#)  [pre-commit.ci] pre-commit autoupdate [ci skip] (:pr:`3255`) :user:`pre-commit-ci`
#)  build(deps): bump actions/setup-python from 6.1.0 to 6.2.0 (:pr:`3254`) :user:`dependabot`
#)  build(deps): bump coverage from 7.13.1 to 7.13.2 (:pr:`3253`) :user:`dependabot`
#)  tests: improve handling of timeout usage (:pr:`3252`) :user:`marcelotduarte`
#)  chore: enable package discovery for src-layout (:pr:`3247`) :user:`marcelotduarte`
#)  build(deps): bump actions/checkout from 5.0.1 to 6.0.2 (:pr:`3251`) :user:`dependabot`
#)  build(deps): bump py-cov-action/python-coverage-comment-action from 3.39 to 3.40 (:pr:`3250`) :user:`dependabot`
#)  build(deps): bump astral-sh/setup-uv from 7.1.6 to 7.2.0 (:pr:`3249`) :user:`dependabot`
#)  build(deps): update lief requirement from >=0.15.1,<=0.17.2 to >=0.15.1,<=0.17.3 (:pr:`3248`) :user:`marcelotduarte`
#)  hooks: add pycparser 3.0 (:pr:`3246`) :user:`marcelotduarte`
#)  hooks: more robust approach for setuptools 80.10.1 (:pr:`3245`) :user:`marcelotduarte`
#)  hooks: add streamlit (:pr:`3214`) :user:`marcelotduarte`
#)  build_exe: include project metadata (dist-info) (:pr:`3242`) :user:`marcelotduarte`
#)  chore: EAFP for unlink and rmtree (:pr:`3243`) :user:`marcelotduarte`
#)  tests: improve modulefinder tests (:pr:`3241`) :user:`marcelotduarte`
#)  hooks: improve pyarrow hook (:pr:`3240`) :user:`marcelotduarte`
#)  Fix KeyError for QmlImportsPath when freezing PyQt5 applications (:pr:`3238`) :user:`realrushen`
#)  tests: make tests pass using pip and mingw (:pr:`3239`) :user:`marcelotduarte`
#)  bdist_msi: refactor to work better with pyproject (:pr:`3235`) :user:`marcelotduarte`
#)  build(deps): update lief requirement from >=0.15.1,<=0.17.1 to >=0.15.1,<=0.17.2 (:pr:`3234`) :user:`marcelotduarte`
#)  hooks: ignore typing_extensions globally and minor tweaks (:pr:`3232`) :user:`marcelotduarte`
#)  hooks: add win32print module (pywin32) (:pr:`3231`) :user:`marcelotduarte`
#)  Bump version: 8.5.2 → 8.5.3 [ci skip] (:pr:`3229`) :user:`marcelotduarte`
#)  hooks: fix setuptools interference in implicit namespaces such as backports.zstd (:pr:`3228`) :user:`marcelotduarte`
#)  fix: avoid possible error w/ pure implicitly namespaces (:pr:`3227`) :user:`marcelotduarte`
#)  chore: Add cooldown for dependabot (:pr:`3226`) :user:`marcelotduarte`
#)  Bump version: 8.5.1 → 8.5.2 [ci skip] (:pr:`3222`) :user:`marcelotduarte`
#)  fix: include-msvcr is also used on MSYS2 environments (:pr:`3219`) :user:`marcelotduarte`
#)  build: fix errors building in conda and msys2 [ci skip] (:pr:`3220`) :user:`marcelotduarte`
#)  [StepSecurity] Apply security best practices (:pr:`3216`) :user:`bot`
#)  hooks: support pyparsing 3.3.x (:pr:`3218`) :user:`marcelotduarte`
#)  security: upgrade dependencies to avoid vulnerabilities detected [ci skip] (:pr:`3217`) :user:`marcelotduarte`
#)  hooks: add numpy 2.4.0 (:pr:`3215`) :user:`marcelotduarte`
#)  build(deps-dev): bump pre-commit from 4.5.0 to 4.5.1 (:pr:`3213`) :user:`dependabot`
#)  build(deps): bump github/codeql-action from 4.31.8 to 4.31.9 (:pr:`3212`) :user:`dependabot`
#)  Bump version: 8.5.0 → 8.5.1 [ci skip] (:pr:`3211`) :user:`marcelotduarte`
#)  build(deps-dev): bump bump-my-version from 1.2.4 to 1.2.5 (:pr:`3210`) :user:`dependabot`
#)  build(deps): bump astral-sh/setup-uv from 7.1.5 to 7.1.6 (:pr:`3208`) :user:`dependabot`
#)  build(deps): bump actions/upload-artifact from 5.0.0 to 6.0.0 (:pr:`3207`) :user:`dependabot`
#)  build(deps): bump actions/download-artifact from 6.0.0 to 7.0.0 (:pr:`3209`) :user:`dependabot`
#)  hooks: fix support for pyproj in conda (:pr:`3203`) :user:`marcelotduarte`
#)  tests: fix tests on windows arm64 [ci skip] (:pr:`3205`) :user:`marcelotduarte`
#)  tests: fix tests using rasterio 1.4.4 (:pr:`3204`) :user:`marcelotduarte`
#)  build(deps): bump github/codeql-action from 4.31.7 to 4.31.8 (:pr:`3202`) :user:`dependabot`
#)  build(deps): bump step-security/harden-runner from 2.13.3 to 2.14.0 (:pr:`3200`) :user:`dependabot`
#)  fix: document changes to Executable.base option (:pr:`3201`) :user:`marcelotduarte`
#)  build(deps): bump coverage from 7.12.0 to 7.13.0 (:pr:`3199`) :user:`dependabot`
#)  build(deps): bump astral-sh/setup-uv from 7.1.4 to 7.1.5 (:pr:`3198`) :user:`dependabot`
#)  hooks: improve scipy hook (:pr:`3197`) :user:`marcelotduarte`
#)  [pre-commit.ci] pre-commit autoupdate [ci skip] (:pr:`3196`) :user:`marcelotduarte`
#)  freezer: ignore .c and .cpp while copy package data (:pr:`3195`) :user:`marcelotduarte`
#)  build(deps): bump github/codeql-action from 4.31.6 to 4.31.7 (:pr:`3194`) :user:`dependabot`
#)  build(deps): bump step-security/harden-runner from 2.13.2 to 2.13.3 (:pr:`3191`) :user:`dependabot`
#)  [pre-commit.ci] pre-commit autoupdate (:pr:`3189`) :user:`pre-commit-ci`
#)  build(deps): bump github/codeql-action from 4.31.5 to 4.31.6 (:pr:`3188`) :user:`dependabot`
#)  build(deps-dev): bump pre-commit from 4.4.0 to 4.5.0 (:pr:`3183`) :user:`dependabot`
#)  build(deps): bump actions/setup-python from 6.0.0 to 6.1.0 (:pr:`3187`) :user:`dependabot`
#)  hooks: fix `import scipy.signal` (:pr:`3186`) :user:`marcelotduarte`
#)  build(deps): bump github/codeql-action from 4.31.4 to 4.31.5 (:pr:`3182`) :user:`dependabot`
