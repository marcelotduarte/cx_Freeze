...
def import_module_or_none(module_name: str) -> ModuleType | None:
    try:
...
def cx_freeze_status() -> tuple[str, bool]:
    ...
    try:
        version = metadata.version("cx_Freeze")
        installed = True
...
def install_requires(
    ...
    *,
    ...
    if test_data:
        ...
        if requirements:
            installed_packages += install_requirements(
                requirements, extra_index_url or [], find_links
            )
...
def pushd(target: StrPath) -> Generator:
    saved = os.getcwd()
    os.chdir(target)
    try:
        yield saved
...
def main() -> int:
    ...
    path = TOP_DIR / "samples" / str(args.sample)
