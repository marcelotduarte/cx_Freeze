...
def cibuildwheel_file() -> Path | None:
    ...
    pattern = (
        f"cx_Freeze-{cx_freeze_version}-{PYTHON_TAG}-"
        f"{PYTHON_TAG}-{PLATFORM_SYSTEM}_*_{PLATFORM_ARCH}.whl"
    )
...
def install_requires(
    ...
    if not PIPENV_ACTIVE and UV is None:
        ...
        if dist:
            if IS_CONDA:
                ...
                if wheel_file is None and not IS_MANYLINUX:
                    # build manylinux or macpython wheel
