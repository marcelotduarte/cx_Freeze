...
get_python () {
    ...
    if ! [ -z $PYTHON ]; then
        local platform=$($PYTHON -c "import sysconfig; print(sysconfig.get_platform(), end='')")
        if [[ $platform == macos* ]]; then
            local py_version=$($PYTHON -c "import sysconfig; print(sysconfig.get_python_version(), end='')")
