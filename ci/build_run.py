...
async def run_gui_capture(output: str) -> list[str]:
    ...
    if IS_MACOS:
        ...
        if CI or not screencapture.exists():
            ...
            screencapture = Path(which("import") or "/usr/sbin/import")
            if screencapture.exists():
                messages.append(
        ...
        screencapture = which(name)
        if screencapture is None:
            screencapture = (
                Path(os.getenv("HOME", "~")).expanduser() / f"bin/{name}.exe"
            )
        else:
            screencapture = Path(screencapture)
        if screencapture.exists():
...
async def run_svc(
    command: tuple[str, ...], output: str  # noqa: ARG001
) -> Process:
    env = dict(os.environ)
    if IS_WINDOWS or IS_MINGW:
        env["PATH"] = r"C:\Windows;C:\Windows\System32"
...
async def main_loop(sample, count, data, debug=False):
    ...
    while count == 0 or (count != 0 and number == count):
        ...
        output = f"{sample}-{number}-{basename}-{PLATFORM}-{PY_VERSION}"
        ...
        if process is not None:
            ...
            print(process.pid, returncode, output, apptype, command_line)
        if debug:
            for ext in ("log", "err"):
...
def main():
    ...
    data = CI_DIR.joinpath("build-test.json").read_text(encoding="utf_8")
    ...
    build_dir = TOP_DIR / f"samples/{sample}/build/exe.{PLATFORM}-{PY_VERSION}"
