...
def get_app(
    data: dict[str, list[str]], sample: str, number: int
    ...
    if IS_WINDOWS or IS_MINGW:
        ...
        popenargs = [app.resolve().as_posix()]
    if args is not None:
        popenargs.append(args)
    return apptype, app.stem, popenargs


def get_command_line(command: Sequence[Path | str]) -> str:
    return " ".join([os.fspath(arg) for arg in command])


@contextlib.contextmanager
def pushd(target: Path | str) -> Generator[str]:
    saved = os.getcwd()
    os.chdir(target)
    try:
        yield saved
...
async def run_cmd(command: Sequence[str], output: str) -> Process | None:
    if command[0] == "true":
        return None
...
async def run_cui(command: Sequence[str], output: str) -> Process:
    env = os.environ.copy()
    if IS_WINDOWS or IS_MINGW:
...
async def run_gui(command: Sequence[str], output: str) -> Process:
    env = os.environ.copy()
    if IS_WINDOWS or IS_MINGW:
        env["PATH"] = r"C:\Windows;C:\Windows\System32"
    elif IS_MACOS and Path(command[0]).parent.name.endswith(".app"):
        command = list(command)
        command[0] = Path(command[0]).parent.as_posix()
...
async def run_svc(command: Sequence[str], output: str) -> Process:  # noqa: ARG001
    env = os.environ.copy()
    if IS_WINDOWS or IS_MINGW:
        env["PATH"] = r"C:\Windows;C:\Windows\System32"
...
async def run_svc_kill(
    command: Sequence[str], output: str, process: Process
) -> Process:
    try:
        await asyncio.wait_for(process.wait(), timeout=TIMEOUT)
    except asyncio.TimeoutError:
...
def validatecode(returncode: int | None, output: str, apptype: str) -> int:
...
async def main_loop(
    sample: str, count: int, data: dict[str, list[str]], debug: bool = False
) -> int:
    ...
    while count == 0 or (count != 0 and number == count):
        appdata = get_app(data, sample, number)
        if appdata is None:
            break
        apptype, basename, appargs = appdata
...
def main() -> int:
    ...
    if IS_WINDOWS or IS_MINGW:
        asyncio.set_event_loop_policy(
            asyncio.WindowsProactorEventLoopPolicy()  # ty: ignore
        )
