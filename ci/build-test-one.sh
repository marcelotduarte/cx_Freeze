        ...
        if [ "$IS_CONDA" == "true" ]; then
            ...
            if ! [ -d $VENV_LOCAL ]; then
                ...
                if which uv &>/dev/null; then
                    uv venv -p $PY_VERSION $VENV_LOCAL
                else
...
if [ "$TEST_DEPS" != "no" ]; then
    # uninstall cx_Freeze
    if [ "$IS_CONDA" == "true" ]; then
        conda remove -n $CONDA_DEFAULT_ENV -y --force-remove cx_freeze || true
    elif [ "$IS_MINGW" == "true" ]; then
        pacman --noconfirm -R ${MINGW_PACKAGE_PREFIX}-python-cx-freeze || true
    fi
    if which uv &>/dev/null; then
...
if [ "$TEST_DEPS" == "--dist" ]; then
    if [ "$IS_CONDA" == "true" ]; then
        ...
        pacman --noconfirm --needed -S ${MINGW_PACKAGE_PREFIX}-python-build
    ...
    if [ "$IS_CONDA" == "true" ]; then
        ...
        pacman --noconfirm --needed -S ${MINGW_PACKAGE_PREFIX}-python-cx-freeze
    ...
    if [ "$IS_CONDA" == "true" ]; then
        conda install -c $PACKAGES_INDEX_URL/conda cx_freeze
    elif [ "$IS_MINGW" == "true" ]; then
        pacman --noconfirm -U ${MINGW_PACKAGE_PREFIX}-python-cx-freeze
    elif which uv &>/dev/null; then
        uv pip install --upgrade -r pyproject.toml
